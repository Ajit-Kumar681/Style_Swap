...
const Wishlist = ({ wishlist, toggleWishlist }) => {
  return (
    <>
      ...
      <div className="page">
        <h1 className="title">My Wishlist ❤️</h1>

        {wishlist.length === 0 ? (
          <p style={{ textAlign: "center", color: "#64748b" }}>No items in wishlist</p>
        ) : (
          <div className="grid">
            {wishlist.map((item) => (
              <div className="card" key={item.id}>
                ...
                <div className="info">{item.title} <br /> ({item.category})</div>
                <button className="remove" onClick={() => toggleWishlist(item)}>Remove</button>
              </div>
            ))}
          </div>
        )}
